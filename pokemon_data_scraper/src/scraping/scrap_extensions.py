...
def get_all_extensions(
    extensions: List[WebElement], blocks: List[WebElement], db_handler: DBHandler, reparse_all: bool = False
    ...
    for idx, block in enumerate(blocks):
        for extension in tqdm(block.find_elements(by=By.CLASS_NAME, value="button")):
            ...
            # Add them for later
            data_blocks.append([extensions[idx].text, ext_code, ext_url, ext_name, ext_image_url, date_release, card_number])
...
def get_all_extensions_beautifulsoup_threading(series_bs: ResultSet, blocks_bs: ResultSet, reprocess_all: bool = False) -> pd.DataFrame:
    ...
    for idx, block in enumerate(blocks_bs):
        r = Parallel(n_jobs=int(config(("THREAD_NUMBER"))), backend="threading")(
            delayed(get_extension_beautifulsoup_threading)(extension, series_bs[idx].text, in_db_extensions) for extension in block.find_all("a")
        )
